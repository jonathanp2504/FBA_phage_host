...
function plot_sensitivity(res_ref, res_alt, title::String, filename::String)
    ...
    enzyme_ylim = (0.0, max(enzyme_max * 1.2, 0.05))
    p_enz = plot(xlabel="t [h]", ylabel="e_mal [-]", ylims=enzyme_ylim,
        legend=:outertop, legend_column=-1; default_style...)
    plot!(p_enz, res_ref.t, res_ref.e_mal, label="Reference", color=:darkorange, lw=2)
    plot!(p_enz, res_alt.t, res_alt.e_mal, label="Alt",       color=:darkred,   lw=2, linestyle=:dash)
...
p_both_alpha_low = make_scenario_params(V_max_both, fbaCache_aerobic; alpha_=alpha_low)
sol_alpha_low    = run_scenario(p_both_alpha_low, [4.44, 2.337, 0.0, 0.0])
res_alpha_low    = extract(sol_alpha_low)

plot_sensitivity(res_both, res_alpha_low,
