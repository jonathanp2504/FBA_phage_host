...
function updatePhageHostRates!(du, u, h, p::Parameters, t)::Nothing
    ...
    # MODEL 4: burst size is dynamisch en afgeleid van de huidige groeisnelheid
    # p.mu_N wordt elke minuut bijgewerkt door fbaUpdate!
    # b(mu_N) = b_0 + k_b * mu_N
    b_dynamic = getBurstSize(p)
    ...
    # b_dynamic in de faagvergelijking
    du[Pfind]  = b_dynamic * (1 - getProbLys(uDecision, p)) * p.k_inject * uLysis[Paind] * uLysis[Nind] / getTotalBiomass(uLysis, p)
