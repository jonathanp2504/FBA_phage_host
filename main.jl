...
include("./Bin/parameters.jl")   # parameters_model4.jl hernoemen
include("./Bin/FBA.jl")          # identiek aan Model 3
include("./Bin/dFBA.jl")         # dFBA_model4.jl hernoemen
include("optimization_2.jl")     # identiek aan Model 3
...
# MODEL 4: kalibratie b_0 en k_b
# Eis: b(mu_max_mal) = b(1.26) ≈ 170 (literatuurwaarde bij optimale groei)
# b_0 + k_b * 1.26 = 170
# Keuze b_0 = 50 (minimale burst size bij nulgroei)
# => k_b = (170 - 50) / 1.26 ≈ 95.2 ≈ 95
# Biologische verantwoording: Wang (2006) Genetics toont lineair verband
# tussen groeisnelheid en burst size voor phage lambda
b_0 = 50.0
k_b = 95.2
...
p = Parameters(
    ...
    tau,
    b_0, k_b,       # twee parameters ipv vaste b
...
println("--- Optimalisatie Model 4 gestart ---")
tspan   = (0.0, duration)
...
p_optimal = Parameters(
    ...
    p.tau, p.b_0, p.k_b,
    p.E_coli_cellDW, p.MW, p.h_release,
