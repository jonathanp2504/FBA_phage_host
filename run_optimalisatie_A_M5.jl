...
function setup_optimizer_A(benz_model,
                            ...
                            rho_DW::Float64,
                            duration::Float64)
    println("=== Setup Optimizer A (Model 5): berekening BENZ_REF ===")
...
function run_optimization_A(p_initial::Parameters;
    ...
    println("\nGrid search A Model 5 ($total combinaties) | biomassa vast = $FIXED_BIOMASSA_A | w_benz=$w_benz")
    for moi in moi_values
        for t_inf in t_inf_values
            counter += 1
            score = evaluate_A(t_inf, moi, p_initial; w_benz=w_benz, w_P=w_P)
            score == 0.0 && continue   # biologisch betekenisloos
    ...
    best_score == -Inf && @warn "Alle grid-punten -Inf — controleer parameters."
    println("\nBeste grid-punt: MOI=$best_moi | t_inf=$best_tinf | score=$(round(best_score,digits=5))")

    println("\nNelder-Mead verfijning A (Model 5)...")
    function f_nm(x)
        s = evaluate_A(x[1], x[2], p_initial; w_benz=w_benz, w_P=w_P)
        return s == 0.0 ? -Inf : s
    end
    ...
    println("\n=== Optimalisatie A (Model 5) voltooid ===")
