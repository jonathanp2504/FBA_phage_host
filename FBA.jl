...
function fbaUpdate!(u, p::Parameters)
    ...
    for i in eachindex(Sind)
        ...
        if haskey(p.fbaModel.reactions, id)
            # STAP A: Bereken de dynamische e_max volgens jouw afleiding
    ...
    if haskey(p.fbaModel.reactions, p.benz_id)
        p.fbaModel.reactions[p.benz_id].lower_bound = geforceerde_benz_flux
        p.fbaModel.reactions[p.benz_id].upper_bound = 1000.0
        # 2. Zet de weg naar buiten open (ESSENTIEEL voor steady-state)
        if haskey(p.fbaModel.reactions, "R_BENZ_export")
