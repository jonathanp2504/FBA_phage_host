...
function setup_optimizer_B(benz_model,
                            ...
                            N0_ref::Float64,
                            duration::Float64)
    println("=== Setup Optimizer B (Model 5): berekening BENZ_REF ===")
...
function run_optimization_B(p_initial::Parameters;
    ...
    println("\nGrid search B Model 5 ($total combinaties) | MOI vast = $FIXED_MOI_B | w_benz=$w_benz")
    ...
    println("\nNelder-Mead verfijning B (Model 5)...")
    function f_nm(x)
        s = evaluate_B(x[1], x[2], p_initial; w_benz=w_benz, w_P=w_P)
        return s == 0.0 ? -Inf : s
    end
    ...
    println("\n=== Optimalisatie B (Model 5) voltooid ===")
