...
open(joinpath(@__DIR__, "plot_analysis.txt"), "w") do io
    @printf(io, "Biomass initial: %.5e, final: %.5e, fold change: %.2f\n", init_b, final_b, fold)
    @printf(io, "Estimated growth rate m = %.4f 1/h, doubling time = %.2f h\n", m, doubling)
    @printf(io, "Glucose initial: %.3f mmol/L, final: %.3f mmol/L\n", init_glc, final_glc)
    @printf(io, "Malate initial: %.3f mmol/L, final: %.3f mmol/L\n", init_mal, final_mal)
    if !isnan(thalf)
        @printf(io, "Glucose reaches half initial (~%.3f) at t = %.2f h\n", half_glc, thalf)
    else
        @printf(io, "Glucose did not drop to half initial within %.1f h\n", duration)
    end
    @printf(io, "\nTime\tBiomass\tGlc\tMal\n")
    for (i,t) in enumerate(time)
        if isapprox(t, round(t); atol=1e-6)
            @printf(io, "%.0f\t%.5e\t%.3f\t%.3f\n", t, biom[i], glc[i], mal[i])
        end
    end
end
