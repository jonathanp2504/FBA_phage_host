...
# dFBA.jl bevat de DEVerbosity patch en laadt OrdinaryDiffEq
# en DelayDiffEq — moet als EERSTE geinclude worden
include("./Bin/dFBA.jl")
...
tau            = 1.32;  b = 170.0;  alfa_ads = 1.3e-9
...
h_release      = 1.71e-12;  duration = 20.0 #zie document hoe ik h_release heb berekend, de oude waarde was 6.0e-12
mu_max_vector  = [0.76, 0.76, 1.10, 0.30] #dit zijn ondertussen de echte waardes (limited medium)
...
# Na sol = run(p_ref), voeg toe:
for i in eachindex(sol.t)
    if sol.t[i] >= 2.0 && sol.t[i] <= 4.0
        u_now = sol.u[i]
        S = u_now[Sind_S]
        P = u_now[Pfind]
        I = u_now[Sind_I]
        L = u_now[Sind_L]
        phi = S > 1.0 ? min(P/S, 10.0) : 0.0
        println("t=$(round(sol.t[i],digits=3))  S=$S  P=$P  I=$I  L=$L  phi=$phi")
    end
end
