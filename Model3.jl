...
# elementen 17-23 zijn ongebruikt maar aanwezig voor compatibiliteit
...
function updatePhageHostRates!(du, u, h, p::Parameters, t)::Nothing
    ...
    # The entry flux alone is not enough: a cohort that entered tau_death
    # hours ago has been growing at mu_l ever since, so it is now larger
    # than it was at entry. Removing only the original entry-sized flux
    # leaves that accumulated growth permanently in the population.
    # Rather than approximating that growth with exp(mu_l * tau_death)
    # (which assumes mu_l was roughly constant over the last tau_death
    # hours -- not reliable if mu_l changed during that window, e.g. right
    # after infection or a substrate switch), the actual growth factor is
    # read directly from the simulation: the ratio of the current
    # lysogenic population to what it was tau_death hours ago tells us
    # exactly how much any cohort has grown in between, since every
    # lysogenic cell grows at the same mu_l regardless of its age.
    ...
    # Lysogenic / producing cells: gain the entry flux now, grow at mu_l,
    # and lose -- entirely, not exponentially -- the cohort that entered
    # exactly tau_death hours ago. There is no more continuous k_tox term.
